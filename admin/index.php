...
<head>
  ...
  <title>Cruise Management System - Admin Login</title>
...
<body class="" style="background-image: url(vendor/pexels-abdullah-ghatasheh-1631677.jpg); " >
  ...
  if (isset($_POST['admin_login'])) {
    ...
    $a_pwd = ($_POST['a_pwd']); //
    $data = mysqli_query($conn, "SELECT a_email, a_pwd, a_id FROM admin WHERE a_email='$a_email' and a_pwd='$a_pwd'"); //sql to log in user
    while ($row = mysqli_fetch_assoc($data)) {
      $_SESSION['a_id'] = $row['a_id']; //assaign session to admin id
      $aaa = $_SESSION['a_id'];
    }
    //$uip=$_SERVER['REMOTE_ADDR'];
    //$ldate=date('d/m/Y h:i:s', time());
    if (isset($aaa)) { //if its sucessfull
